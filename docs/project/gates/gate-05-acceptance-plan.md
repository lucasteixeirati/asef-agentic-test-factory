...
- **Estado:** execução em andamento; 5.7 publicado em `v0.1.0a5`; candidata local 5.8/`0.1.0a6` pronta para CI; publicação e 5.9 pendentes
...
|---|---|---|---|
| G5-01 | Wheel instala e executa fora do checkout | sessão limpa + CI de package audit | Atendido: wheel e sdist `0.1.0a5` auditados, instalação limpa aprovada e pré-release publicada |
...
| G5-05 | Análise, riscos, cenários, testes e resultados são rastreáveis | report + schema/contract tests | Atendido localmente no 5.8.6: cadeia tipada, parser e JSON Schema Draft 2020-12 reconciliados no wheel; CI pendente |
...
| G5-14 | Reports JSON e Markdown separam fatos, inferências e recomendações | schema + revisão de conteúdo | Atendido localmente no 5.8.6: JSON normativo, Markdown derivado e auditor instalado 9/9; CI pendente |
...
| G5-17 | Core não importa Python tooling, Docker, OpenAI ou LangGraph | import boundaries + job core mínimo | Atendido até `v0.1.0a5`: fronteiras AST e job core aprovados |
| G5-18 | README, quickstart, tutorial, arquitetura, segurança e limitações refletem o Alpha real | auditoria documental | Atendido localmente no 5.8.6: checker sem findings e walkthrough frio completo; execução pública do job pendente |
...
| G5-20 | CI pública e regressões do Gate 4 permanecem verdes | execução GitHub Actions | Parcial: regressões locais 5.8 verdes; última CI pública 5.7 aprovou seis jobs, nova matriz de sete aguarda checkpoint |
